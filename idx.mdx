...
Here's the complete contract we'll build, including both transfer and approval indexing:

```typescript listeners/src/Listener.sol [expandable]
pragma solidity ^0.8.19;

import {ProbeDsl, ChainIdContract} from "simidx/Dsl.sol";
import {
    Usdc$function_OnPostTransfer,
    Usdc$function_transferInputs,
    Usdc$function_transferOutputs,
    Usdc$postTransferFunction,
    Usdc$function_OnPostApprove,
    Usdc$function_approveInputs,
    Usdc$function_approveOutputs,
    Usdc$postApproveFunction
} from "simidx/generated/Usdc.sol";

contract UserProbe is Usdc$function_OnPostTransfer, Usdc$function_OnPostApprove, ProbeDsl {

    // Events for the data we want to index
    event Transfer(address indexed to, uint256 value);
    event Approval(address indexed spender, uint256 value);

    constructor() {
        // Hook for transfer events
        addHook(
            ChainIdContract(
            ...
            Usdc$postTransferFunction(),
            UserProbe.onPostTransfer.selector
        );

        // Hook for approval events
        addHook(
            ChainIdContract(1, 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48),
            Usdc$postApproveFunction(),
            UserProbe.onPostApprove.selector
        );
    }

    function onPostTransfer(
        Usdc$function_transferInputs memory inputs,
        Usdc$function_transferOutputs memory outputs
    ) external override {
        if (outputs.outArg0 == true) {
...
We'll build this contract step by step in the following sections, starting with the basic transfer functionality and then adding approval indexing.
...
Here's the complete API we'll build with all endpoints:

```typescript apis/src/index.ts [expandable]
...
This is the complete API we'll build step by step. It includes endpoints for both transfers and approvals, connecting to your indexed database to serve the data through REST endpoints.
