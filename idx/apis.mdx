...
Let's create our first endpoint to fetch the 10 most recent Uniswap V3 pools. This endpoint will query the `poolcreated` table we created in our listener:
...
Now let's add an endpoint to fetch the 10 most recent owner queries. This will query the `ownerqueried` table:
