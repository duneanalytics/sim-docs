...
Let's modify the default API to create dedicated endpoints for the `pools` and `owner` data we indexed in the [Index On-Chain Data](./index-on-chain-data) guide.
...
app.get("/api/pools", async (c) => {
  try {
    ...
    const result = await client.execute('SELECT * FROM pool_created ORDER BY block_number DESC LIMIT 10');
...
app.get("/api/owner-queries", async (c) => {
  try {
    const client = await getDBClient(c.env);
    const result = await client.execute('SELECT * FROM owner_queried ORDER BY block_number DESC LIMIT 10');
    return Response.json({
      data: result.rows,
    });
  } catch (e) {
