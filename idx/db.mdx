...
When you inspect your database, you will see both the clean views you should query and the internal tables with random suffixes. **Always query the views (lowercase `snake_case` names) and remember that every column name follows the same `snake_case` convention.**
