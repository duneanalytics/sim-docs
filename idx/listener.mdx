...
If you want to add an additional ABI, first find the ABI for that contract by going to sites like Etherscan, then run `sim abi add YourContract.json`. That command will automatically generate structs, interfaces, and functions for use in your Listener.
...
The `lib/simidx/src/` directory contains `Simidx.sol`, which provides the core Sim IDX framework including the DSL for triggers and auto-generated interfaces based on your ABIs.
...
Your listener logic resides in `listeners/src/Listener.sol`.
...
```solidity
// Inherits from the DSL and a generated interface for createPool
...
constructor() {
    addTrigger(
        ChainIdContract(1, 0x1F98431c8aD98523631AE4a59f267346ea31F984), // Target
        UniswapV3Factory$postCreatePoolFunction(), // Function to watch
        Listener.onPostCreatePool.selector // Handler to call
    );
}
```
...
This command generates the necessary interfaces, structs, and helper functions in `listeners/lib/simidx/src/generated/`, allowing you to use it in your Listener.
    ...
    override
{
    ...
    emit PoolCreated(
        ...
        inputs.tokenB, 
        inputs.fee,
        block.number  // Add the current block number
    );
}
```

This modification will create a new column `blockNumber` in your `pools_created` table, allowing you to track exactly when each pool was created.
You can use this same pattern to add any additional data you want to capture in your events.

## Indexing More Functions

Let's extend the sample app to also index calls to the `owner()` view function on the Uniswap V3 Factory contract.

<Steps>
<Step title="Extend the Contract Interface">
Update your listener contract to implement the generated interface for the `owner` function. The `UniswapV3Factory$function_OnPostOwner` interface is already available because the ABI was included in the sample app.

```solidity
contract Listener is 
    UniswapV3Factory$function_OnPostCreatePool, 
    UniswapV3Factory$function_OnPostOwner, // Add this interface
    ListenerDsl 
{
    // ...
```
...
In the `constructor`, add a new trigger for the `owner` function.
...
constructor() {
    ...
    addTrigger(
        ...
        UniswapV3Factory$postOwnerFunction(), // Generated helper for owner()
...
Define a new event to structure the owner data. This will create an `owner_queried` table.
...
Create the corresponding handler function. Since `owner()` is a view function with no inputs, the handler only receives an `outputs` struct.

```solidity
function onPostOwner(
    UniswapV3Factory$function_ownerOutputs memory outputs
) external override {
    // The owner() function returns a single address
...
<Tip>
Install the [Hardhat VSCode plugin](https://marketplace.visualstudio.com/items?itemName=NomicFoundation.hardhat-solidity) for Solidity code completion. It will help you discover the available generated interfaces, structs, and helpers as you type.
</Tip>

## Testing Your Listener

Sim IDX uses Foundry for testing. You can write unit tests for your listener in `listeners/test/Listener.t.sol` to ensure it behaves as expected.

To run your tests, use the `sim test` command:

```bash
sim test
```

This will compile your contracts and run all tests defined in the test file, providing immediate feedback on your listener's logic.
