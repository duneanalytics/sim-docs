...
<Card
  title="Learn More About PostgreSQL Database Indexes"
...
To add a database index, use a special comment with the `@custom:index` annotation directly _above_ the `event` definition in your Solidity listener.
...
<Card
  title="Learn More About PostgreSQL Index Types"
