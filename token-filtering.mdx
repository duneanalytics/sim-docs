...
Include certain tokens regardless of their liquidity metrics by maintaining a list of approved token symbols.

```javascript
// Allowlist of trusted token symbols (uppercase recommended)
const ALLOWLISTED_SYMBOLS = [
  'USDC', // USD Coin
  'DAI',  // DAI Stablecoin
  'WBTC', // Wrapped Bitcoin
];

const filterWithAllowlist = (tokens) => {
  return tokens.filter(token => {
    // Always include allowlisted tokens
    if (token.symbol && ALLOWLISTED_SYMBOLS.includes(token.symbol.toUpperCase())) {
      return true;
    }
    // Apply other filtering criteria for non-allowlisted tokens
    return token.pool_size && token.pool_size >= 1000;
  });
};
```

Allowlisting is particularly useful for ensuring major tokens like stablecoins and established cryptocurrencies are always displayed to users.

### Denylisting Specific Tokens

Exclude certain tokens even if they meet your liquidity criteria by maintaining a blocklist of problematic symbols.
