...
The Token Info endpoint has a fixed CU cost of **1** per request. The `chain_ids` query parameter is required but does not change the CU cost. See the [Compute Units](/compute-units) page for detailed information.