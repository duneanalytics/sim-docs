...
<Note>
  This endpoint is currently in beta and requires authentication using a normal sim API key. The `chain_ids` query parameter is mandatory.
</Note>
...
To fetch tokens for all supported chains, pass the `chain_ids=all` parameter. See [EVM Supported Chains](/evm/supported-chains) for more details. You can also specify specific chains with `?chain_ids=11,250,1`.
