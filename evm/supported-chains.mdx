...
- **All chains**: `?chain_ids=all` (returns every chain supported by that endpoint)
...
Untagged chains stay out of default requests to keep them fast, but you can still query them with `chain_ids` by passing the chain name (e.g. `?chain_ids=corn,funkichain`) or by requesting `all`.
