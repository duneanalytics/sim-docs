...
- **`token_address`** — Filter by token contract address. For example, pass the USDC contract address to return only USDC-related activity.
- **`activity_type`** — Filter by activity type. Accepted values: `send`, `receive`, `mint`, `burn`, `swap`, `approve`, `call`.
- **`asset_type`** — Filter by asset standard. Accepted values: `native`, `erc20`, `erc721`, `erc1155`. Use `native` to get only native token transfers (e.g. ETH). Contract call activities have no asset type and are excluded when this filter is set.
