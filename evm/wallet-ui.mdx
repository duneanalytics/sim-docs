...
By the end of this guide, you will have a wallet displaying token balances and their aggregated USD value.
...
<Frame caption="The Token Balances tab and Total Portfolio Value we'll build in this guide.">
  <img src="https://mintlify.s3.us-west-1.amazonaws.com/sim-dune/images/wallet-ui/balances-with-logos.png" className="mx-auto" style={{ width:"100%" }} />
</Frame>

## Project Setup

Let's start by scaffolding our project.
This initial setup will provide a basic Express.js server and HTML templates, allowing us to focus on integrating Dune's sim APIs.
...
<Steps>
  ...
  <Step title="Add Starter Code">
    ...
    let mockTokens = [];
    let mockActivities = []; // For Guide 2
    let mockCollectibles = []; // For Guide 3
    ...
    app.get('/', async (req, res) => {
        ...
        // In future steps, we will replace mockData with API calls
        res.render('wallet', {
            walletAddress: walletAddress,
            currentTab: tab,
            totalWalletUSDValue: '$0.00', // Will be calculated
            tokens: mockTokens,
            activities: mockActivities, // For Guide 2
            collectibles: mockCollectibles, // For Guide 3
            errorMessage: null
        });
    });
...
Start your server by running `node server.js` in the terminal. Visit `http://localhost:3001` in your browser. You should see a blank wallet page. To load data, you can provide a wallet address as a query parameter, for example: `http://localhost:3001/?walletAddress=0xd8da6bf26964af9d7eed9e03e53415d37aa96045`. The wallet address field is also clickable for input.

Now, let's integrate the sim API to fetch real data.

### Fetching and Displaying Token Balances

We will use the [Balances API](/evm/balances) to get realtime token balances for a given wallet address. This endpoint provides comprehensive details about native and ERC20 tokens, including metadata and USD values across more than 60+ EVM chains.

First, let's create an asynchronous function in `server.js` to fetch these balances. Add this function before your `app.get('/')` route handler:

```javascript server.js (getWalletBalances)
async function getWalletBalances(walletAddress) {
    ...
    try {
        ...
        // The API returns balances in data.balances
        // We also format the balance for easier display
...
This function `getWalletBalances` constructs the API request URL, includes your `DUNE_API_KEY` in the headers, and sends a GET request to the `/echo/v1/balances/evm/{address}` endpoint.
The Balances API endpoint gives you access to various URL query parameters that you can attach to the request to modify the output.
We have included `metadata=url,logo` to enrich our data with token logos and `exclude_spam_tokens=true` to filter out common spam tokens.
You can find more details about available parameters in the [Balances API documentation](/evm/balances).

Now, modify your `app.get('/')` route handler in `server.js` to call `getWalletBalances` and pass the fetched tokens to your template:
...
2.  Calculate `totalWalletUSDValue` by summing the `value_usd` of each token. The `balances` API provides this pre-calculated value for each asset.
3.  Pass the `tokens` and formatted `totalWalletUSDValue` to the `wallet.ejs` template.

The `views/wallet.ejs` file you created earlier is already set up to display these tokens and the total value. Restart your server (`node server.js`) and refresh your browser, providing a `walletAddress` in the URL.
For example: `http://localhost:3001/?walletAddress=0xd8da6bf26964af9d7eed9e03e53415d37aa96045`

You should now see the wallet populated with token balances, logos, and the calculated total portfolio value for the specified address.

<Frame caption="Wallet displaying token balances with logos and total USD value.">
  <img src="https://mintlify.s3.us-west-1.amazonaws.com/sim-dune/images/wallet-ui/balances-with-logos.png" className="mx-auto" style={{ width:"100%" }} />
</Frame>

<Warning>
  If you encounter errors, ensure your `.env` file contains the correct `DUNE_API_KEY` and that it is loaded correctly. Also, verify the `walletAddress` in the URL is a valid EVM wallet address. Check your terminal for any error messages from `server.js`.
</Warning>

### Conclusion

Congratulations! You have successfully set up the basic structure of your multichain wallet and integrated Dune's sim APIs `balances` endpoint to display real-time token balances and total portfolio value. You learned how to:
*   Set up a Node.js project with Express.js and EJS.
*   Fetch token balances using the `/echo/v1/balances/evm/{address}` endpoint.
*   Use query parameters like `metadata` and `exclude_spam_tokens` to refine API results.
*   Calculate and display the total USD value of the wallet's portfolio.

In the next guide, [Guide 2: Tracking Wallet Activity](./build-a-realtime-wallet-p2-activity), we will enhance this wallet by adding a transaction history feature using the `activity` API endpoint.
