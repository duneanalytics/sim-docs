...
<Steps>
  ...
  <Step title="Add Starter Code">
    ...
    app.get('/', async (req, res) => {
        ...
        // In future steps, we will replace empty arrays with API calls
        res.render('wallet', {
            walletAddress: walletAddress,
            currentTab: tab,
            totalWalletUSDValue: '$0.00', // Will be calculated in the next section
            tokens: tokens,
...
2. Pass the retreived `balances` to the `wallet.ejs` template.
...
<Frame caption="Wallet displaying properly formatted token balances with logos and total USD value.">
...
Let's calculate the total USD value of the wallet properly show it.
