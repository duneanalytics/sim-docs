...
<Frame caption="Token balances with twenty four hour price change indicators showing gains and losses.">
  <img
    src="/images/wallet-ui/wallet-historical-prices.png"
    className="mx-auto"
    style={{ width:"100%" }}
    alt=""
    title=""
...
```javascript server.js (getWalletBalances with historical prices) [expandable] {5,9,16,17,18,19,20,21,22,23,24}
/**
...
```javascript server.js (imports and route updates) [expandable] {4,18,30,49}
import express from 'express';
import numbro from 'numbro';
import dotenv from 'dotenv';
...
```ejs views/wallet.ejs (price change badges)
...
```css public/styles.css (price change styles) [expandable]
.price-delta-badges {
    display: flex;
    gap: 6px;
