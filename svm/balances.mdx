...
| raw_balance                    | Raw balance from account info divided by 10^decimals         | string        |
...
<Note>
The `amount` field is based on `uiAmountString` from the account info (not the raw `amount`) to maintain backward compatibility. The new `raw_balance` field provides the raw `amount` divided by 10^decimals, which usually matches `balance` except when contracts implement scaling logic.
</Note>
